...
### Phase 2 — DI swap: Hilt → Koin (unblocks everything)
6. Add Koin (`koin-core`, `koin-compose`, `koin-compose-viewmodel`).
7. Per module, replace `@Module @InstallIn`/`@Provides`/`@Binds` with Koin `module { }` definitions; `@Inject constructor` → `factory/single { ... }`; `@Singleton` → `single`. Qualified dispatchers (`@Dispatcher(FsmDispatchers.IO)`) → Koin named qualifiers.
8. `hiltViewModel()` → `koinViewModel()`; `@HiltViewModel` → plain ViewModel registered as `viewModel { }`. `FlowSixApplication` → `startKoin { }`; iOS `initKoin()` called from `MainViewController`.
9. Keep this Android-only first (still single-target) so the app builds green on Android with Koin before adding iOS. **This is the biggest edit surface (18 files).**
